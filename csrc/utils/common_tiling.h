...
{
    OP_CHECK(tilingData.opShape.m0 > 0, "m0 is invalid", return false);
    OP_CHECK(tilingData.opShape.k0 > 0, "k0 is invalid", return false);
    OP_CHECK(tilingData.opShape.n0 > 0, "n0 is invalid", return false);
    OP_CHECK(tilingData.mLoop > 0, "mLoop is invalid", return false);
    OP_CHECK(tilingData.kLoop > 0, "kLoop is invalid", return false);
    OP_CHECK(tilingData.nLoop > 0, "nLoop is invalid", return false);
    OP_CHECK(tilingData.blockDim > 0, "nLoop is invalid", return false);
    return true;
}
} // namespace AsdOps
#endif